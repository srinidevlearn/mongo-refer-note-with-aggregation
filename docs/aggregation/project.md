...
- (not similar to one available with find operator)
- once we specify one field to retain we must specify all fields to retain, but _id is exceptional
- we can add new fields apart from removing or retaining fields
- we can use $project whenever needed within aggregation pipeline(including n number of times)
- we can use this for reassign values to existing field names and derive new fields.
